...
def test_india_kit_uses_paper_broker_and_disables_unreliable_news(tmp_path):
    kit = main.build_kit(Settings(market="india", universe="market"), sessions(tmp_path))
...
def test_india_watchlist_mode_has_no_screener(tmp_path):
    kit = main.build_kit(Settings(market="india", universe="watchlist"), sessions(tmp_path))
    assert kit.screener is None


def test_india_needs_no_alpaca_keys(monkeypatch):
...
def test_us_market_still_requires_alpaca_keys(monkeypatch):
    for k in ("ALPACA_API_KEY", "ALPACA_SECRET_KEY"):
        monkeypatch.delenv(k, raising=False)
    monkeypatch.setenv("OPENROUTER_API_KEY", "k")
    monkeypatch.setenv("MARKET", "us")
    with pytest.raises(SystemExit, match="ALPACA_API_KEY"):
        main.build_pipeline(live=False)
...
def test_india_screener_uses_the_nse_list_and_indian_thresholds(tmp_path):
    ...
    screener = main.build_india_screener(settings)
...
def test_report_prints_paper_account_summary(tmp_path, capsys):
    ...
    main.print_report(Settings(market="india", database_url=url, paper_initial_cash=250_000.0))
    out = capsys.readouterr().out
    assert "250,000" in out and "closed trades 0" in out and "₹" in out


def test_report_is_refused_for_us_market(tmp_path):
    with pytest.raises(SystemExit):
        main.print_report(Settings(market="us"))


def test_india_kit_analyses_completed_sessions_and_quotes_live(tmp_path):
    kit = main.build_kit(Settings(market="india"), sessions(tmp_path))
...
def test_us_pipeline_registers_technical_and_sentiment_agents(monkeypatch, tmp_path):
    monkeypatch.setenv("OPENROUTER_API_KEY", "k")
    ...
    monkeypatch.setenv("TELEGRAM_BOT_TOKEN", "")
    pipeline = main.build_pipeline(live=False)
    assert list(pipeline.agents) == ["technical", "sentiment"]
...
def test_print_graphs_lists_the_agents_for_each_market(capsys):
    main.print_graphs(Settings(market="india"))
    india = capsys.readouterr().out
    main.print_graphs(Settings(market="us"))
    us = capsys.readouterr().out
    assert "agent_technical" in india and "agent_sentiment" not in india
    assert "agent_technical" in us and "agent_sentiment" in us
